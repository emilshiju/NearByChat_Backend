...
export class UserRepository implements IUserRepository {
  async create({ userName, dob, gender, email, password }: User): Promise<userList> {
  ...
  async findUser(data: findUser): Promise<MyObject[]> {
    ...
    const userLocation:LocationType|null = await locationModel.findOne({ userId: data.userId });
  ...
  async RuserLogin(email: string, password: string): Promise<userList|boolean> {
  ...
  async RuserStatus(userId:string): Promise<boolean> {
   ...
   async RsaveLocation(longitude: number, latitude: number,userId:string): Promise<boolean> {
...
async RsendOtp(email: string, otp: string): Promise<userOtp> {
  ...
  const a=await  otpModel.create({
   ...
   return a.toObject()
    ...
    async RoleBasedAuthentication(id: mongoose.Schema.Types.ObjectId): Promise<userList|null> {
   ...
   async ReditUserDetails(userId:string,userName: string, dob: string, gender: string): Promise<userList|null> {
    ...
    const resposne:userList|null=await UserModel.findOneAndUpdate({_id:userId}
      ,
      {
        $set:{
          userName:userName,
   ...
   async RgetOrderSummary(userId: string): Promise<SubscriptionType[]> {
    ...
    const response:SubscriptionType[]=await paymentSummaryModel.find({userId:userId})
   ...
   async RchangePassword(userId:string,password: string): Promise<userList|null> {
     
    try{
    
    const response:userList|null=await UserModel.findOneAndUpdate({_id:userId},
      {$set:{
        password:password
      }
      }
