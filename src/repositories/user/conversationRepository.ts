...
export class conversationRepository implements IConversationRepository {
  async RConversation(
    input: Conversation
  ): Promise<CombinedType | null | userList> {
...
// @ts-ignore
  ...
  async RGetAllConversation(userId: string): Promise<any> {
    ...
    const lastAttempt: any[] = await Promise.all(
      chatrooms.map(async (a: any) => {
    ...
    const allfiltered = lastAttempt.filter(
      (item) => item.all._id !== undefined
    );
  ...
  async RDelteChat(selectedUserId: string, userId: string): Promise<null> {
    try {
      const response: saveChatRoom | null = await chatRoomModel.findById(
        selectedUserId
      );
